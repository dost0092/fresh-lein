...
async function getUserFromRequest(req) {
  ...
  if (!token) return { user: null, token: null, error: null };
  try {
    const supabase = getAdminClient();
    const { data, error } = await supabase.auth.getUser(token);
    if (error || !data?.user) return { user: null, token: null, error: error?.message || 'invalid_token' };
    return { user: data.user, token, error: null };
  } catch (err) {
    if (String(err?.message || '').includes('Missing SUPABASE')) {
      return { user: null, token: null, error: 'server_misconfigured' };
    }
    return { user: null, token: null, error: err?.message || 'auth_failed' };
  }
}
